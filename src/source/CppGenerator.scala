...
class CppGenerator(spec: Spec) extends Generator(spec) {
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface) {
    ...
    spec.cppNamespace match {
    case None => inherit = s" : public virtual Base"
    case Some(s) => inherit = s" : public virtual $s::Base"
    }
    ...
    writeHppFile(ident, origin, refs.hpp, refs.hppFwds, w => {
      ...
      w.w(s"class $self" + inherit).bracedSemi {
        ...
        val className = q(classLookup+"$CppProxy")
        w.wlOutdent("public:")
        w.wl(s"virtual const char* getCppProxyClassName() {return $className;}")
        w.wl
        // Destructor
  ...
  def generateCommonHeaderFile(): Unit = {
    createFile(spec.cppHeaderOutFolder.get, s"Common.${spec.cppHeaderExt}", w => {
      w.wl("// AUTOGENERATED FILE - DO NOT MODIFY!")
      w.wl("// This file generated by Djinni")
      w.wl
      w.wl("#pragma once")
      w.wl
      wrapNamespace(w, spec.cppNamespace, w => {
        w.wl("class Base").bracedSemi {
          w.wlOutdent("protected:")
          w.wl("virtual const char * getCppProxyClassName() = 0;")
        }
      })
    })
  }
